...
**Interlock baseline:** this repository at `c548bb0` (origin/main at audit time; includes
#59, the gate item 2 crash-window single-writer proof on C2, and #58, the S8 `MessageBus`
worker-outbound endpoint discharging gate item 6). No relevant PR was open when this audit
was finalised.
...
| D14 | Pre-dispatch context checks: ambiguity/OS-precondition checklist, committed-base file-existence check, contracts grep, parallel same-file collision briefing, release pre-fetch | `.claude/skills/org-delegate/SKILL.md:98-134`; `.claude/skills/org-delegate/references/delegate-flow-details.md:82-127`; `.claude/skills/org-delegate/references/release-pre-fetch.md:1-26` | **U** →G2 | Delegation-quality disciplines of the operating layer; folded into G2's brief/delegation contract. |
...
| M10 | Read-only human observation of live sessions (dispatcher viewer, org-attach read-only attach command generation) | `README.md:118,141`; `.claude/skills/org-attach/SKILL.md:25-31`; `docs/operations/dispatcher-view.md:1-9` | **U** (human observation, →G9) + **DR** (pane-attach mechanics) | The pane-attach *mechanics* are Discard-bucket machinery (D-0009, D-0014), and the machine-facing half is substituted by the provider's `read_state`/readouts (S1). But a machine API is not human observation: the human-facing capability — watching what the organisation is doing, live — has no existing substitute (G9 is explicitly unimplemented), so that half is an undesigned gap, not a reduction. |
...
| P6 | Completion-report contract: full-depth reports carry a 3-part human-understanding summary used as the human approval surface; minimal-depth is a 1-line fast path; mandatory pre-completion rebase-clean check | `.claude/skills/org-delegate/SKILL.md:352`; `.claude/skills/org-delegate/references/worker-claude-template.md:100-108,157-167`; `docs/contracts/role-contract.md:208-212` | **U** →G2 | Part of the brief/delegation contract (what a worker owes back); folded into G2. |
...
(`.claude/skills/org-delegate/references/delegate-flow-details.md:82-127`), and fixes what the worker owes back (the
...
- **Priority:** split, and honestly grounded — no acceptance criterion makes human
  response latency a canary go/no-go (Q-0005 leaves canary criteria open; Q-0011/gate
  item 8 concern Secretary responsiveness under load, a different thing). The
  **signalling half is needed-before-canary** on an operating-parity argument: v1 runs
  every one of these gates with an attention channel, so a canary run silently waiting
  hours on an unnoticed approval would pollute the AC-7 shadow-divergence comparison with
  human-absence noise and leave canary escalations unnoticed. The **notification-backend
  half can follow**: for one canary worker, the already-shipped stdout/bell fallback on
  the carried watcher is a workable minimum once the signalling exists; restoring a
  desktop backend remains Q-0017's decision.
