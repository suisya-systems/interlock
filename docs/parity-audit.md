...
| T3 | Triage repo-set resolution from the registry (`triage` opt-out column; `triage_home` opt-in flag) | `registry/projects.md:17-21`; `registry/org-config.md:43-53`; `tools/work_discovery_repos.py:1-15` | **U** →G7 | Travels with T2. |
...
| M6 | Attention watcher: classify human-required events (approval blocked, CI failed, pending decision, silent stop) from durable state, dedup, notify | `docs/operations/attention-watch.md:1-17`; `tools/templates/attention.example.json:1-27` | **C** (classification) + **DR** (desktop backend, →G5) | Carried and shipped: `attention scan|watch` with a pure no-I/O classifier and dedup (`src/claude_org_runtime/attention/__init__.py:3-9`; ledger rows for `attention/classifier.py`, `attention/dedup.py`). The desktop notification backend was deleted with `attention/platform.py` (`PORTING_LEDGER.md` purge record: "a capability is lost, deliberately"; notifications resolve to a stdout line) and reinstating one is a recorded open question, Q-0017. |
...
| M13 | Org bring-up/teardown lifecycle (org-start parallel boot, org-down gated daemon teardown) | `.claude/skills/org-start/SKILL.md:199-226`; `.claude/skills/org-down/SKILL.md:36-102` | **DR** | `org up` / `org down` around interactive TUI panes is a Discard row (`PORTING_LEDGER.md` row for `src/claude_org_runtime/broker/launcher.py`; D-0009, D-0010). Substitute: Interlock's supervisor owns worker processes directly (C2, D-0025/D-0027); what "bringing up the successor org" means operationally is part of G2/G10's operating-layer scope. |
...
| C10 | Citation-or-abstain discipline for the human window | `CLAUDE.md:5-14` | **U** | Prose discipline of the operating layer's human window; no mechanism to port and no draft — carried into the successor's Secretary material when G2/G10 write it. |
| C11 | State-file schema versioning and migration framework (per-file versions, tolerant readers, central migration registry) | `docs/contracts/state-schema-contract.md:202-228`; `tools/state_migrate.py:1-9` | **C** | The migration/comparison bridge is a rewrite row (`PORTING_LEDGER.md` row for `src/claude_org_runtime/migrate/v1_to_v2.py`; D-0013 — bridge only, never a permanent API); shipped `src/claude_org_runtime/migrate/__init__.py:5-9`. Schema revision handling is fail-closed (refuse, don't migrate silently — `control_plane/schema.py`). |
...
branch work is cut from and merged to (`registry/projects.md:6-10,24-30`). v1 keeps this in
...
**Gap rows:** M5, M6 (backend half; Q-0017).
...
classifier and dedup but deleted the desktop backend deliberately (purge record for
...
- **undesigned:** 32 rows, consolidated into 10 drafts (G1–G10) — 6 needed before the
  ...
  folded into G2's scope.
- **undetermined:** 4 ledger rows (M7, D15, C7, C12); §3 lists 5 items in total — U1, U3,
  U5 correspond to ledger rows one-to-one, U2 gathers M7/T6/K2 under Q-0019, and U4 is a
  convention with no single ledger row. Each is tied to an open question (Q-0011, Q-0019,
  Q-0022) or an operator preference, deliberately not answered by this audit.
