...
**Class is not purge status.** A row's class records what the *fork source* path is. Whether the
path still exists in this repository is a separate fact, recorded in
[Purge record](#purge-record). A `discard` row whose file is still present has not been
reclassified — it is retained, deferred, or not yet reached.
...
That walk is **historical**: it describes the fork source tree at the base commit. It is *not* a
claim about the present contents of this repository — see [Purge record](#purge-record).
...
|---|---|---|---|
| `src/claude_org_runtime/broker/store.py` | carry (invariant) / rewrite (mechanism) | Queue store + append-only journal implementing `UNDELIVERED → CLAIMED → DELIVERED` with lease-based claim/reap, mode-epoch fencing, idempotent `DELIVERED`, and lock-outside-I/O journal discipline. No send-keys or screen-scraping coupling found, but the file is **not** pane-free, and the coupling is in the **adoption-rollback** path specifically: `AdoptRollback.detached_panes` records the panes an adopt detached, and when an adoption expires the `pane_gone` guard — computed from whether `_reattach_owner_panes_locked` got those panes back — decides whether the *previous delivery instance* is restored. (The observer lease is released before that check; pane presence conditions the instance rollback, not lease expiry.) So one delivery-ownership invariant here is *conditioned on pane presence*, via collaborator hooks `_detach_owner_panes_locked` / `_reattach_owner_panes_locked` that the store calls under `_lock` but does not implement. That is the session/delivery entanglement D-0009 separates, and it must be re-derived onto a transport-neutral liveness signal rather than carried. With that carved out, the remaining **delivery invariants** are the Carry bucket's "message identity, ack, dedup invariants" and "lease, outbox, resume conditions" almost line for line. But the **storage mechanism cannot be carried**: the authoritative `_rows` is in memory and the file states outright that the journal is not replayed to reconstruct state ("`_rows` は in-memory・journal replay で再構築しない (crash recovery なし)"). A daemon kill therefore loses queue state, which contradicts D-0001 and would fail the outbox/resume fault-injection gate. Override of the mechanical survey's flat `carry` — the invariants port, the store is re-derived on SQLite. | D-0001, D-0009, D-0014 |
...
| Path | Class | Rationale | Decisions |
|---|---|---|---|
| `src/claude_org_runtime/attention/dedup.py` | carry (invariant) / rewrite (mechanism) | Carries the two dedup namespaces with their differing semantics — record-once-forever for events, cooldown-gated for pending decisions — which is the Carry bucket's dedup invariant and what the incident dedup key must preserve. **Its corruption handling must not carry:** "a broken state file recovers as empty" was safe when the state was an advisory notification ledger, but once dedup state is durable and authoritative, treating it as empty forgets every already-handled message or incident and permits previously applied effects to replay — a direct violation of resume-without-double-execution. The rewritten store must fail closed or rebuild from durable records instead. The key's actual composition remains open (`Q-0002`). | D-0001, D-0007, D-0009, D-0014 |
...
| `src/claude_org_runtime/attention/readers.py` | rewrite | What carries is only the half of the invariant that says a missing file, absent table, or corrupt DB must **not crash the watcher**. The mechanism by which it does so — collapsing failure into an *empty result* — must **not** carry: an empty list is indistinguishable from a successful read that found nothing, so the classifier could never raise `OBSERVATION_UNAVAILABLE` as distinct from `NO_ACTIVITY_EVIDENCE`, which is precisely the separation D-0006 requires. The rewritten reader must return a typed unavailable/error result. The schemas read here (`events` table, `pending_decisions.json`, the broker `queue.jsonl` tail) all predate the SQLite SoT tables in any case. | D-0001, D-0005, D-0006, D-0014 |
...
| Path | Class | Rationale | Decisions |
|---|---|---|---|
| `src/claude_org_runtime/settings/generator.py` | carry (invariant) / rewrite (mechanism) | Schema-driven generation of per-role `settings.local.json` (permissions, sandbox, hooks). The **fencing renderer** is what the Carry bucket names — per-role permission / sandbox / hooks generation and validation — and it carries. Two things in the same module do **not**: it imports `..transport.descriptor` (classified `discard` in this ledger, so carrying the module intact either preserves that contract or leaves a broken dependency), and it carries the A / B / C `sandbox_by_pattern` machinery (32 occurrences) that D-0014 discards along with the old worker layout. Override of the mechanical survey's flat `carry`: split the renderer from the discarded transport and pattern axes. | D-0009, D-0014, D-0017 |
...
| `src/claude_org_runtime/schema/enums.py` | rewrite | The canonical-name discussion (`task_id` vs `worker`, `pane_id` vs `pane_name`) appears **only in the module docstring** — the normalisation itself is implemented in `migrate/v1_to_v2.py`, not here. What this file actually defines is the v1 vocabulary: `WorkerStatus`, `JournalEventType`, and an ad-hoc `AnomalyKind`, including pane events — precisely the free-vocabulary model that the closed fact-state set replaces (D-0005). Override of the mechanical survey's `carry`, which took the docstring's framing for the file's content: the naming *discipline* is quarry, the enums are re-derived. | D-0001, D-0005, D-0014 |
...
| Path | Class | Rationale | Decisions |
|---|---|---|---|
| `tests/broker/test_delivery.py` | carry (invariant) / rewrite (mechanism) | The largest and most directly relevant test file in the tree: claim-then-confirm state machine, claim-respecting drain, **lease-reap recovery**, **mode-epoch fencing against stale confirms**, claim-issuance gating, and delivery-scoped credential isolation — simultaneously the message-identity/ack/dedup invariant set and the fault-injection matrix the acceptance gate names. **It is not pane-free.** Across its 118 test functions, `spawn_claude_pane` / `spawn_pane` / `add_pane` / `close_pane` appear as 59 identifier nodes in the parsed AST (docstrings and comments therefore excluded) and `FakeAdapter` is imported from the `discard`-classed `tests/broker/conftest.py`; the file also pins old backend contracts outright — `test_spawn_omits_kind_for_backends_without_the_capability` and `test_venv_wrapper_backend_is_unchanged` exist to hold the tmux / wezterm / herdr spawn signatures still, and `test_stale_lease_is_released_when_the_pane_died_out_of_band` — the one test in the file that calls `kill_pane` — makes stale-lease release *depend on* an adapter's answer about whether a pane died. That subset is the same pane-nudge surface the `tests/broker/test_store.py` row drops, and it goes with it; the lease-release invariant underneath it needs a transport-neutral liveness signal before it can be re-pinned (`Q-0023`). Mechanism half: re-target the surviving delivery assertions from `store.py`'s API to the `MessageBus` contract. | D-0001, D-0009, D-0014 |
| `tests/broker/test_store.py` | carry (invariant) / rewrite (mechanism) | Delivery-lifecycle assertions carry — registered-binds-only targets, name-to-id resolution, at-most-once drain (3 of its 6 tests). The other three are nudge-worker tests driven by a fake terminal adapter against canned idle / busy Claude-TUI screens, and they are dropped with pane nudging. **The single-flight guard belongs to that dropped half, not the carried one**: `test_nudge_single_flight_under_concurrent_sends` asserts on `Broker._nudge_threads` under a busy *screen*, so it pins the nudge worker's thread hygiene rather than a `MessageBus` invariant. The no-double-injection *intent* is quarry for the outbox's at-most-once send, but this test does not carry. | D-0009, D-0014 |
| `tests/broker/test_control_plane.py` | carry (invariant) / rewrite (mechanism) | The carried half is **narrow and must be stated as such**. Five of its fifty test functions carry: the discovery-metadata/secret separation (`daemon.json` round-trip and field contract, `state_dir` absolutised, `admin.token` written atomically at `0600`, idempotent removal) and the offset-scoped journal verification that stops a prior run's line from reading as a false positive — resume-condition and verification-probe material, following `sidecar.py`. The last three (`test_pid_alive_*`) carry on a separate ground: they pin `sidecar.pid_alive` against a live pid, a reaped child, and malformed input, which is the process-liveness contract any resume path needs and is not admin-RPC surface at all. **The terminal-backend field is carved out of the carried set**: `test_sidecar_backend_none_for_no_nudge` asserts only that the legacy `--no-nudge` configuration records `backend=None` and drops whole, while `test_sidecar_roundtrip_and_fields` — carried otherwise — writes and asserts `backend="tmux"` among its fields, and that one assertion drops with the daemon-file shape. The forty-odd functions between them exercise the admin HTTP RPC — token mint and tier, adopt/handover, `flip_mode`, SIGTERM and shutdown — against `broker/server.py`, `broker/tokens.py`, `broker/surface.py` and `tests/broker/conftest.py`, **all four classified `discard` in this ledger**, and they do not carry. The literal token `pane` occurs zero times here, which is not the same as transport-neutrality: the `backend` assertions above are terminal-backend coupling with no `pane` in them. | D-0001, D-0014 |
| `tests/broker/test_residents.py` | carry (invariant) / rewrite (mechanism) | Process-identity fault injection through injected platform seams — `observe` / `alive` / `kill` / `sleep` / `os_name` / `proc_root` all arrive as parameters, so no real process is signalled. The claim stops there: the leaf helpers `residents._hostname` and `residents._clock_ticks` are **not** injectable and are replaced by twelve `monkeypatch.setattr` calls on module globals, so "no patched globals" does not hold for the file as a whole. Making those two seams injectable is part of the mechanism half. Mirrors the `residents.py` classification otherwise. | D-0001, D-0014, D-0017 |
...
| `tests/broker/__init__.py` | discard | Empty package marker. **Retained under interlock#39**: duplicate test basenames (`test_notify.py` exists under both `tests/attention/` and `tests/broker/`) need the package marker under pytest's prepend import mode. | D-0014 |

### `tests/attention/`

| Path | Class | Rationale | Decisions |
|---|---|---|---|
| `tests/attention/test_dedup.py` | carry (invariant) / rewrite (mechanism) | Carries the once-ever vs cooldown-gated dedup invariants, which are Carry-bucket material. Does **not** carry its corruption assertions: `test_load_recovers_from_broken_json` and `test_load_recovers_from_non_object` both pin "malformed state loads as an empty `DedupState`", which is exactly the behaviour the `attention/dedup.py` row above rules out once dedup state is durable and authoritative — an empty ledger permits already-applied effects to replay. Those two cases must be rewritten to assert fail-closed or durable-rebuild instead. | D-0001, D-0007, D-0014 |
...
| `tests/attention/__init__.py` | discard | Empty package marker. **Retained under interlock#39**: the kept `tests/attention` tests use `from .conftest import ...`. | D-0014 |

### `tests/` (root) and other test packages

| Path | Class | Rationale | Decisions |
|---|---|---|---|
| `tests/test_settings_generator.py` | carry (invariant) / rewrite (mechanism) | Verification tests for the permission / sandbox / hooks rendering that carries. Tracks its module: the cases covering the discarded `transport.descriptor` allowlist derivation and the A / B / C `sandbox_by_pattern` axis are rewritten or dropped with those mechanisms, while the per-role fencing and breach-probe cases carry. | D-0009, D-0014, D-0017 |
...
| `tests/__init__.py`, `tests/scrub/__init__.py` | discard | Empty package markers (0 lines each), listed for completeness so that every one of the 53 `.py` files under `tests/` has a verdict. Re-created as needed by the new package layout rather than ported. **Retained under interlock#39**: `tests/scrub/test_scrub.py` imports `tests.scrub` absolutely, so these markers are what put the repo root on `sys.path` under pytest's default prepend import mode. | D-0014 |
| `tests/transport/__init__.py` | discard | Empty package marker, split out of the grouped row by interlock#39 because its fate differs: `tests/transport/` holds no kept test once `test_descriptor.py` goes, so the marker was deleted with the directory. | D-0014 |
...
| `docs/cli.md` | discard | Reference documentation for a CLI surface whose subcommands are almost entirely `rewrite` or `discard`; documentation is a quarry, not a porting source. **Carved out under interlock#39**: the `settings generate` / `settings show` / `sandbox doctor` sections document `settings/sandbox_doctor.py` (carry) and the carried half of `settings/generator.py`, so the file is trimmed to its still-true sections rather than deleted (D-0015). | D-0014, D-0015 |
...
`discard` rows do not delete themselves. This section records what
[interlock#39](https://github.com/suisya-systems/claude-org-runtime/issues/39) actually did to the
working tree, under the authority of **D-0014** (the classification) and **D-0015** (which forbids
cutting a `carry` or `rewrite` row, or a safety fence, to reduce LOC). No class cell anywhere above
was changed by this purge — see [Class is not purge status](#legend).

Four purge actions are used, and only these four:

| Action | Meaning |
|---|---|
| `deleted` | The path no longer exists in this repository. |
| `content stripped, file kept` | The file survives because deleting it would break a `carry` or `rewrite` row; the discard **content** inside it is gone. |
...
| `docs/broker-bootstrap-stage1-folder-trust-design.md` | `discard` | `deleted` |  |
| `src/claude_org_runtime/attention/platform.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/broker/__main__.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/broker/cli.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/broker/launcher.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/broker/placement.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/broker/server.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/broker/surface.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/broker/tokens.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/prompts/__init__.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/prompts/templates/curator.md` | `discard` | `deleted` |  |
| `src/claude_org_runtime/prompts/templates/dispatcher.md` | `discard` | `deleted` |  |
| `src/claude_org_runtime/prompts/templates/secretary.md` | `discard` | `deleted` |  |
| `src/claude_org_runtime/schema/json_schema/__init__.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/schema/json_schema/broker_queue_event.schema.json` | `discard` | `deleted` |  |
| `src/claude_org_runtime/schema/json_schema/journal_event.schema.json` | `discard` | `deleted` |  |
| `src/claude_org_runtime/schema/json_schema/worker_dir_entry.schema.json` | `discard` | `deleted` |  |
| `src/claude_org_runtime/schema/org_state.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/terminal/__init__.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/terminal/base.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/terminal/herdr.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/terminal/keys.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/terminal/tmux.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/terminal/wezterm.py` | `discard` | `deleted` |  |
| `src/claude_org_runtime/transport/descriptor.py` | `discard` | `deleted` |  |
| `tests/attention/test_platform.py` | `discard` | `deleted` |  |
| `tests/broker/conftest.py` | `discard` | `deleted` |  |
| `tests/broker/test_bootstrap_folder_trust.py` | `discard` | `deleted` |  |
| `tests/broker/test_channel_sentat_drop.py` | `discard` | `deleted` |  |
| `tests/broker/test_channel_sidecar.py` | `discard` | `deleted` |  |
| `tests/broker/test_cli.py` | `discard` | `deleted` |  |
| `tests/broker/test_launcher.py` | `discard` | `deleted` |  |
| `tests/broker/test_nudge_misroute.py` | `discard` | `deleted` |  |
| `tests/broker/test_placement.py` | `discard` | `deleted` |  |
| `tests/broker/test_schema.py` | `discard` | `deleted` |  |
| `tests/broker/test_server.py` | `discard` | `deleted` |  |
| `tests/broker/test_space_layout.py` | `discard` | `deleted` |  |
| `tests/broker/test_surface.py` | `discard` | `deleted` |  |
| `tests/terminal/__init__.py` | `discard` | `deleted` |  |
| `tests/terminal/conftest.py` | `discard` | `deleted` |  |
| `tests/terminal/test_base.py` | `discard` | `deleted` |  |
| `tests/terminal/test_herdr.py` | `discard` | `deleted` |  |
| `tests/terminal/test_keys.py` | `discard` | `deleted` |  |
| `tests/terminal/test_tmux.py` | `discard` | `deleted` |  |
| `tests/terminal/test_venv.py` | `discard` | `deleted` |  |
| `tests/terminal/test_wezterm.py` | `discard` | `deleted` |  |
| `tests/test_dispatcher_multitab_geometry.py` | `discard` | `deleted` |  |
| `tests/test_dispatcher_multitab_placement.py` | `discard` | `deleted` |  |
| `tests/test_dispatcher_multitab_population.py` | `discard` | `deleted` |  |
| `tests/test_dispatcher_multitab_portability.py` | `discard` | `deleted` |  |
| `tests/test_dispatcher_runner.py` | `discard` | `deleted` |  |
| `tests/test_prompts.py` | `discard` | `deleted` |  |
...
| `tests/broker/test_control_plane.py` | `carry (invariant) / rewrite (mechanism)` | `quarantined` | Module-level `pytest.importorskip("claude_org_runtime.broker.server")` inserted; every assertion kept verbatim at its ledgered path. The file drives `broker/server.py`, which this purge deletes, so no runnable subset exists — see `Q-0023`. It self-heals the day a `broker.server` replacement lands. |
| `tests/broker/test_delivery.py` | `carry (invariant) / rewrite (mechanism)` | `quarantined` | Module-level `pytest.importorskip("claude_org_runtime.broker.server")` inserted; every assertion kept verbatim at its ledgered path. The file drives `broker/server.py`, which this purge deletes, so no runnable subset exists — see `Q-0023`. It self-heals the day a `broker.server` replacement lands. |
| `tests/broker/test_notify.py` | `rewrite` | `quarantined` | Module-level `pytest.importorskip("claude_org_runtime.broker.server")` inserted; every assertion kept verbatim at its ledgered path. The file drives `broker/server.py`, which this purge deletes, so no runnable subset exists — see `Q-0023`. It self-heals the day a `broker.server` replacement lands. |
| `tests/broker/test_store.py` | `carry (invariant) / rewrite (mechanism)` | `quarantined` | Module-level `pytest.importorskip("claude_org_runtime.broker.server")` inserted; every assertion kept verbatim at its ledgered path. The file drives `broker/server.py`, which this purge deletes, so no runnable subset exists — see `Q-0023`. It self-heals the day a `broker.server` replacement lands. |
| `tests/test_migrate.py` | `rewrite` | `quarantined` | Three of its nine tests reach `schema.json_schema` / `schema.org_state`, both deleted. Those three acquire the modules through a local skip guard instead of a module-level import; no assertion is removed. See `Q-0023`. |
...
The purge recorded in [Purge record](#purge-record) removes only `discard` rows, so no figure in
this table moves.
...
| `Q-0022` | Whether a `carry`-classed artifact may keep enumerating v1 pane vocabulary as a normative list — raised by `tests/scrub/scrub_fixture.py` and `docs/scrub-policy.md`, which pin `pane_id` / `pane_name` as never-modified structural identifiers. |
| `Q-0023` | Whether a carried test may drive a `discard`-classed module to reach the contract it pins — raised by the hybrid rows that instantiate `Broker` from `broker/server.py`. **interlock#39 forced the question and answered it provisionally, not finally**: the five affected files are `quarantined` (see [Purge record](#purge-record)) — assertions kept verbatim, skipped rather than run — because deleting them would cut a carried row, keeping `broker/server.py` alive would defeat the purge, and a stub `Broker` would re-vendor a discarded module. The question stays open until those invariants are re-targeted onto the rewritten message bus. |
