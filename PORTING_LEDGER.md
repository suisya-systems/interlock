...
| `tests/broker/test_control_plane.py` | carry (invariant) / rewrite (mechanism) | The carried half is **narrow and must be stated as such**: five of its fifty test functions cover the discovery-metadata/secret separation (`daemon.json` fields, `admin.token` written atomically at `0600`, idempotent removal) and the offset-scoped journal verification that stops a prior run's line from reading as a false positive. `test_sidecar_backend_none_for_no_nudge`, which sits among them, is **not** one of them — it asserts only that the legacy `--no-nudge` terminal configuration records `backend=None`, which is the discarded terminal-backend contract, and it drops with the daemon-file shape. Those are resume-condition and verification-probe material and they follow `sidecar.py`. The last three (`test_pid_alive_*`) carry too, on a separate ground: they pin `sidecar.pid_alive` — a live pid, a reaped child, and malformed pid inputs — which is the process-liveness contract any resume path needs and is not admin-RPC surface at all. The forty-odd functions between them exercise the admin HTTP RPC — token mint and tier, adopt/handover, `flip_mode`, SIGTERM and shutdown — against `broker/server.py`, `broker/tokens.py`, `broker/surface.py` and `tests/broker/conftest.py`, **all four classified `discard` in this ledger**, and they do not carry. No pane coupling anywhere in the file (0 occurrences). | D-0001, D-0014 |
