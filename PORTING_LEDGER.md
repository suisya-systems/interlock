...
|---|---|---|---|
| `src/claude_org_runtime/broker/store.py` | carry | Queue store + append-only journal implementing `UNDELIVERED → CLAIMED → DELIVERED` with lease-based claim/reap, mode-epoch fencing, idempotent `DELIVERED`, and lock-outside-I/O journal discipline. No `PaneId` / send-keys / screen coupling found. This is the Carry bucket's "message identity, ack, dedup invariants" and "lease, outbox, resume conditions" almost line for line. | D-0001, D-0009, D-0014 |
