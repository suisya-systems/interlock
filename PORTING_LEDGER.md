...
| Path | Class | Rationale | Decisions |
|---|---|---|---|
| `tests/broker/test_delivery.py` | carry (invariant) / rewrite (mechanism) | The largest and most directly relevant test file in the tree: claim-then-confirm state machine, claim-respecting drain, **lease-reap recovery**, **mode-epoch fencing against stale confirms**, claim-issuance gating, and delivery-scoped credential isolation — simultaneously the message-identity/ack/dedup invariant set and the fault-injection matrix the acceptance gate names. **It is not pane-free.** Across its 118 test functions there are 77 `spawn_claude_pane` / `spawn_pane` / `add_pane` / `close_pane` call sites and `FakeAdapter` is imported from the `discard`-classed `tests/broker/conftest.py`; the file also pins old backend contracts outright — `test_spawn_omits_kind_for_backends_without_the_capability` and `test_venv_wrapper_backend_is_unchanged` exist to hold the tmux / wezterm / herdr spawn signatures still. That subset is the same pane-nudge surface the `tests/broker/test_store.py` row drops, and it goes with it. Mechanism half: re-target the surviving delivery assertions from `store.py`'s API to the `MessageBus` contract. | D-0001, D-0009, D-0014 |
